...
from .utils.plots import plot_bearing_load
...
class Logger(object):
    ...
    def update_stats(self, target, pred, batch_size, loss, lr, time_used, params, **kwargs):
        self._iter += 1
        self._iter_total += 1
    ...
    def draw_graph_pred(self, cur_epoch):
        true, pred = torch.cat(self._true).cpu().numpy(), torch.cat(self._pred).cpu().numpy()
        if cfg.tensorboard_each_run:
            if cfg.task.type == 'load':
                rot =  torch.cat(self._custom_stats['rot']).detach().cpu().numpy()
                f = plot_bearing_load(pred, true, rot)
                self.tb_writer.add_figure('figures/load', f, global_step=cur_epoch)
