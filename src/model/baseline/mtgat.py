...
class ConvLayer(nn.Module):
    ...
    def __init__(self, n_features, 
                 kernel_size=7, groups=1, 
                 add_pad=True, padding=0, 
                 dilation=1, stride=1,
                 activation='relu'
                 ):
...
class Head(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, out_dim, n_layers, dropout, activation='relu'):
...
class MTGAT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if encode:
            groups = input_dim if cfg.model.mtgat.encoder.in_depth else 1
            self.encoder = ConvLayer(input_dim, kernel_size, groups, 
                                     add_pad=cfg.model.mtgat.encoder.conv_pad, 
                                     dilation=dilation, 
        ...
        self.head = Head(gru_hid_dim, n_layers=head_n_layers, 
                         out_dim=output_dim, hid_dim=head_hid_dim, dropout=0., activation=activation)
    ...
    def forward(self, batch, return_graph=False):
        ...
        if return_graph:
            ...
            return out, edge_index, feat_att.view(-1, 1)
