...
from src.dataset.dataset import load_data_and_create_dataloader
...
def run_seed(args):
    ...
    loaders, scaler = load_data_and_create_dataloader(cfg.task.type)
    model = create_model(cfg.model.type)
    optimizer = create_optimizer(model.parameters())
    scheduler = create_scheduler(optimizer, cfg.optim.warmup)
    ...
    trainer.lazy_init(next(iter(loaders[0])))
    logging.info(summary(model))
    
    if neptune_writer is not None:
